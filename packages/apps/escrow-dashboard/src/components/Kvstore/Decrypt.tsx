          ...
          >
            ...
            <Box><Typography>Decrypt </Typography></Box>
              <Box display={`flex`} flexDirection={"row"} alignContent={`center`}>
                  {privkey.length===0 && <Button sx={{my:2}} disabled={loading} variant="outlined" size="small" component="label">
                      Upload Private Key
                      <input
                          type="file"
                  ...
                  {privkey.length>0 &&<><Typography sx={{my:2}} variant={`body1`}>{filename}</Typography><CloseIcon  onClick={()=>setPrivkey("")} sx={{cursor:'pointer',my:2.3}}/></>}
              </Box>
              <TextField
                  sx={{ my: 1 }}
                  id="outlined-basic"
